...
import Mob from './controllers/mob.js';
...
server.post(['/','/form','/new'], fileParser(), Firebase.company, Mob.toDbObject, async (req, res) => {
    let mob = res.mob;
    // console.debug(mob);

    //# ACTIONS AFTER MOB OBJECT CONSTRUCTION
    await Firebase.storeCase(mob);                      // store the case in the database, mob object is modified here
    SendEmail.aboutNewMob(mob);                     // send email, do not await the delivery
    Mob.deleteAttachments(mob);                     // delete attachments from disk, do not await
    res.render('newcaseconfirm',{mob});                 // render the confirmation page
});


// Αναζήτηση υπόθεσης με το ID και το PIN από το χρήστη
server.post('/case', async (req, res) => {
    let mob = await Firebase.getCase(req.body.id, req.body.pin??"no");    // pin="no" is wrong on purpose (in case no pin provided)
    // mob is now the case object or null
    if (mob) {
        Firebase.markMessagesAsRead(mob);     // mark messages as read. Do not await
        res.render('viewcase', {mob: Mob.toHumanFormat(mob)});
    } else {
...
server.post('/pushmessage', fileParser(), Mob.messageConstructor, async (req, res) => {
    let message = res.message;
    try{
        //# ACTIONS AFTER USER MESSAGE
        // if mobID is invalid, then the following will throw an error
        let mob = await Firebase.pushMessageByUser(message);    // push the message to the database
        SendEmail.aboutNewUserMessage(mob);                     // send email, do not await the delivery
        Mob.deleteAttachments(message);                         // delete attachments from disk, do not await
        res.render('newmessageconfirm',{mob});                  // render the confirmation page
    } catch (e) {
        console.error(e);
...
server.post('/notifyuser', async (req, res) => {
    try{
        ...
        let mob = await Firebase.getCase(body.caseId);      // mob object or null
        if ( !mob || user.companyID!=mob.companyID ) { return res.status(404).json("Δεν βρέθηκε η υπόθεση") }

        SendEmail.aboutCaseUpdate(mob);     // do not await. Returns true (sent) or false (without sumbitter email)
...
server.get("/test-new", (req, res) => {
    let mob = {id: 1234567890123456, pin: 1234, companyID: "bkueHt76TQiUW7G8p1BK"}
    res.render('newcaseconfirm',{mob});
});
server.get('/test-pushmessage', async (req, res) => {
    ...
    res.render('newmessageconfirm',{mob: {id: message.caseId, companyID: "bkueHt76TQiUW7G8p1BK"}});
});
server.get("/test-case", (req, res) => {
    // let mob = JSON.parse('{"date":"2024-03-14","submitter":{"firstname":"ΔΗΜΗΤΡΗΣ","phone":"+306948060820","email":"dvainanidis@gmail.com","notify":true,"lastname":"ΒΑΪΝΑΝΙΔΗΣ"},"companyID":"bkueHt76TQiUW7G8p1BK","pin":"0679","isTest":false,"origin":"http://127.0.0.1","messages":[],"description":"Κλέβει το ταμέιο","filenames":["movie.mp4","voter_list.csv"],"id":"5250467415406000","people":"Μήτσος","status":"initial","submittedAt":{"_seconds":1709405787,"_nanoseconds":149000000}}');
    let mob = {"date":"Λιγότερο από 5 χρόνια πριν","submitter":{"firstname":"ΔΗΜΗΤΡΗΣ","phone":"6948060820","email":"dvainanidis@gmail.com","notify":true,"lastname":"ΒΑΪΝΑΝΙΔΗΣ"},"companyID":"bkueHt76TQiUW7G8p1BK","isTest":false,"origin":"http://127.0.0.1","description":"Άντε πάλι ο Μπάμπης","id":"5862227586012496","people":"Ο Μπάμπης ο Σουγιάς","submittedAt":{"_seconds":1709728584,"_nanoseconds":917000000},"title":"Με απ' όλα","type":"","pin":"0000","filenames":["ChordPage Data.xlsx","unsubs.txt","ENFIA-.pdf"],"status":"under resolution","messages":[{"role":"Υπεύθυνος","readByUser":true,"user":"vainanidis@computerstudio.gr","date":{"_seconds":1709843230,"_nanoseconds":371000000},"text":"Αυτό παίζει αλήθεια;"},{"readByCompany":true,"date":{"_seconds":1710260687,"_nanoseconds":154000000},"role":"Καταγγέλλων","text":"Ναι βρε σου λέω","filenames":[]},{"readByCompany":true,"text":"να, κι ένα αρχείο","role":"Καταγγέλλων","filenames":["unsubs.txt"],"date":{"_seconds":1710260715,"_nanoseconds":128000000}}]};
    res.render('viewcase', {mob: Mob.toHumanFormat(mob)});
});
...
const mob = onRequest({ region , maxInstances: 2 , concurrency: 8 }, server);
...
export { mob , userCreation , afterCaseDeleted , casesCleanup };
